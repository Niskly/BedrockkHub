  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .thumb{
      ...
      overflow: hidden; /* Ensure blurry background doesn't overflow */
    }
    .thumb .pack-icon-bg { /* New: Blurry background image */
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
    ...
    .thumb .pack-icon { /* New: Main square icon */
      position: relative; /* Bring icon to front */
      width: 90px; /* Adjust size as desired */
      height: 90px; /* Adjust size as desired */
...
<script type="module">
  ...
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndoeG1mcGRtbnN1bmdjd2xmZmR4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTYzMDk3MzYsImexciI6MjA3MTg4NTczNn0.PED6DKwmfzUFLIvNbRGY2OQV5XXmc8WKS9E9Be6o8D8';
  ...
  window.openModal = function(){
    ...
    state.modalResolution = '16x';
    state.modalTags = new Set();
    buildSwatches(document.getElementById('modalSwatches'), (k, el) => { state.modalColor = k; highlightOne(el.parentElement, el); }, new Set(['blue']));
    buildTags(document.getElementById('modalTags'), (k, el) => { toggleSet(state.modalTags, k); el.classList.toggle('active'); }, new Set());
    buildResolutionBoxes(document.getElementById('modalResolutions'), (res, el) => { state.modalResolution = res; highlightOne(el.parentElement, el); }, '16x');
  }
  ...
  window.handleUpload = async function(e){
    ...
    if(!state.modalResolution){ alert('Pick a resolution'); return; }
  ...
  window.hintFile = function(){
    ...
    if(f){ document.getElementById('fileHint').textContent = f.name; }
  }
  ...
  // UPDATED: renderCard now shows the custom icon with a blurry background
